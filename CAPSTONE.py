...
daftar_film = {
    'Insidious' : {'Genre' : 'Horror', 'Durasi' : 120, 'Kategori Usia' : 'D 17+', 'Jumlah Kursi' : 15, 'Harga Tiket' : 40000},
    'Ice Age' : {'Genre' : 'Animation', 'Durasi' : 110, 'Kategori Usia' : 'SU', 'Jumlah Kursi' : 15, 'Harga Tiket' : 35000},
    'Mission Impossible' : {'Genre' : 'Action', 'Durasi' : 160, 'Kategori Usia' : 'D 17+', 'Jumlah Kursi' : 20, 'Harga Tiket' : 50000},
    'The Avengers' : {'Genre' : 'Action', 'Durasi' : 135, 'Kategori Usia' : 'R 13+', 'Jumlah Kursi' : 20, 'Harga Tiket' : 45000}, 
    'Jumanji' : {'Genre' : 'Comedy', 'Durasi' : 130, 'Kategori Usia' : 'R 13+', 'Jumlah Kursi' : 25, 'Harga Tiket' : 45000},
    'Adventure Of Tintin' : {'Genre' : 'Animation', 'Durasi' : 120, 'Kategori Usia' : 'R 13+', 'Jumlah Kursi' : 20, 'Harga Tiket' : 40000},
    'The Nun' : {'Genre' : 'Horror', 'Durasi' : 110, 'Kategori Usia' : 'R 17+', 'Jumlah Kursi' : 25, 'Harga Tiket' : 50000},    
    'Planet Earth' : {'Genre' : 'Documentary', 'Durasi' : 180, 'Kategori Usia' : 'SU', 'Jumlah Kursi' : 30, 'Harga Tiket' : 60000}    
}
daftar_key = ('Nama Film', 'Genre', 'Durasi', 'Kategori Usia', 'Jumlah Kursi', 'Harga Tiket')
...
def print_all(nama_dict):
    if len(nama_dict) != 0:
        print( '='*95)
        print(f'{daftar_key[0]: <25}|{daftar_key[1]: <15}|{daftar_key[2]: <10}|{daftar_key[3]: <15}|{daftar_key[4]: <15}|{daftar_key[5]: <10}')
        print('-------------------------+---------------+----------+---------------+---------------+-----------')
        for i in nama_dict.keys():
            print(f'{i: <25}|{(daftar_film[i][daftar_key[1]]): <15}|{(daftar_film[i][daftar_key[2]]): <10}|{(daftar_film[i][daftar_key[3]]): <15}|{(daftar_film[i][daftar_key[4]]): <15}|{(daftar_film[i][daftar_key[5]]): <10}')
        print( '='*95)
    else:
        print(Fore.RED +'\n\t--Data Film Kosong (╥﹏╥)--')
...
def sort_column(nama_dict, column, cond):
    nama_dict_sorted = {k: v for k, v in sorted(nama_dict.items(), key=lambda x:x[1][column], reverse= cond)}
    return nama_dict_sorted
...
def filter_min_max(column):
    ...
    if filter_min == filter_max:
        ...
        for judul in daftar_film.keys():
            if int(filter_min) <= daftar_film[judul][column] and int(filter_max) >= daftar_film[judul][column]:
                hasil_filter[judul] = daftar_film[judul]
...
def create_one():
    ...
    while duplicate or empty:
        judul_film = input('\nMasukkan judul film baru yang ingin ditambahkan: ').title()
        duplicate = False
        empty = False
...
def create_many():
    while True:
        print('Masukkan jumlah data film yang ingin dibuat (max 5)\nMasukkan angka 0 untuk kembali ke Menu Create ')
        num = input()
        if not num.isdigit():
            print(Fore.RED +'\nInput harus berupa angka (╥﹏╥)\n')
        else:
...
def update_all(judul_film):
    ...
    print(f'{daftar_key[0]: <25}|{daftar_key[1]: <15}|{daftar_key[2]: <10}|{daftar_key[3]: <15}|{daftar_key[4]: <15}|{daftar_key[5]: <10}')
    print('-------------------------+---------------+----------+---------------+---------------+-----------')
    print(f'{judul_film: <25}|{(daftar_film[judul_film][daftar_key[1]]): <15}|{(daftar_film[judul_film][daftar_key[2]]): <10}|{(daftar_film[judul_film][daftar_key[3]]): <15}|{(daftar_film[judul_film][daftar_key[4]]): <15}|{(daftar_film[judul_film][daftar_key[5]]): <10}')
    print( '='*95)
    ...
    print(f'{daftar_key[0]: <25}|{daftar_key[1]: <15}|{daftar_key[2]: <10}|{daftar_key[3]: <15}|{daftar_key[4]: <15}|{daftar_key[5]: <10}')
    print('-------------------------+---------------+----------+---------------+---------------+-----------')
    print(f'{judul_film:<25}|{daftar_film_copy[judul_film][daftar_key[1]]:<15}|{daftar_film_copy[judul_film][daftar_key[2]]:<10}|{daftar_film_copy[judul_film][daftar_key[3]]:<15}|{daftar_film_copy[judul_film][daftar_key[4]]:<15}|{daftar_film_copy[judul_film][daftar_key[5]]:<10}')
    print('='*95)
    while True:
